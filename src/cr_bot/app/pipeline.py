...
def process_frames_batch(
    ...
    for frame in frames:
        ...
        crop_h = int(frame_h * fh)
        arena_px = (crop_x, crop_y, crop_w, crop_h)
        prepared.append((frame, frame_to_analyze, arena, arena_px))

    raw_results = detector.infer_batch([item[2] for item in prepared])
    outputs = []
    for (frame, frame_to_analyze, arena, arena_px), result in zip(prepared, raw_results):
        yolo_boxes = result.get_data()
        tower_hp_yolo_boxes = getattr(result, "untracked_data", yolo_boxes)
        yolo_boxes = remap_boxes_to_frame(
            yolo_boxes,
