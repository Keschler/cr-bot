...
def _evaluation_quality_gate(
    ...
    revision_guard = report.get("revision_guard")
    # Reports generated by the current runner always carry this guard. Keep
    # hand-built/injected legacy reports compatible, while failing a report
    # that explicitly records mixed committed revisions closed.
    code_revision_stable = not isinstance(revision_guard, Mapping) or (
        revision_guard.get("status") == "stable"
        and revision_guard.get("checkpoint_matches_run") is not False
    )
    checks = {
        ...
        "code_revision_stable": code_revision_stable,
    }
    ...
    if not checks["code_revision_stable"]:
...
class _CheckpointMatchRunner:
    ...
    def metadata_report(self) -> dict[str, object]:
        return {
            ...
            "checkpoint_code_revision": self.metadata.get("code_revision"),
...
def run_evaluation_matrix(
    ...
    *,
    ...
    checkpoint_revision = runner_metadata.get("checkpoint_code_revision")
